...
let InternalBenefitsService = class InternalBenefitsService {
    ...
    matchPremium(pricing, additionalInsureds, tenureYears) {
    ...
    async getMyInsurance() {
        ...
        if (contactId == null)
            return { noProfile: true, startDate: null, elections: [] };
        const [profileRows, electionRows] = await Promise.all([
            this.dataSource.query(`SELECT TOP 1 CONVERT(varchar(10), ep.StartDate, 120) AS startDate
         FROM dbo.EmployeeProfile ep WHERE ep.ContactID = @0
         ORDER BY ep.EmployeeProfileID DESC`, [contactId]),
        ...
        const elections = electionRows.map((row) => {
            ...
            return {
                ...
                additionalInsureds: row.additionalInsureds != null ? String(row.additionalInsureds).trim() : null,
                healthPlanId,
                planName: row.planName != null ? String(row.planName).trim() : null,
                monthlyPremium: this.matchPremium(pricing, row.additionalInsureds != null ? String(row.additionalInsureds) : null, tenureYears),
                pricing,
            };
        });
    ...
    async listPlans() {
        const [planRows, benefitRows] = await Promise.all([
            this.dataSource.query(`SELECT hp.HealthPlanID AS healthPlanId, hp.PlanName AS planName, hp.PlanType AS planType
         FROM dbo.HealthPlan hp
         WHERE hp.IsActive = 1
         ORDER BY hp.PlanType, hp.PlanName`),
            this.dataSource.query(`SELECT hpb.HealthPlanID AS healthPlanId, hpb.BenefitDescription AS benefitDescription
        ...
        const pricingByPlan = await this.loadCurrentPricing(planRows.map((row) => Number(row.healthPlanId)));
        const benefitsByPlan = new Map();
...
exports.InternalBenefitsService = InternalBenefitsService = __decorate([

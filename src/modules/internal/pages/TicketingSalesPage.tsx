import { Calendar, Landmark, Plus, Ticket } from "lucide-react";
...
const TICKETING_TEAM_MEMBERS: TeamMember[] = [
  { name: "Andy Friedlander", title: "Ticketing Manager" },
  { name: "Tony Ranieri", title: "eCommerce Operations Manager" },
];
...
const TICKETING_EVENTS: DepartmentEvent[] = [
...
const TICKETING_PAGE = {
  ...
  teamMembers: TICKETING_TEAM_MEMBERS,
  quickLinks: TICKETING_QUICK_LINKS,
  events: TICKETING_EVENTS,
  ...
  return (
    ...
    >
      <div className="mx-auto grid min-h-[300px] max-w-[1120px] items-center gap-8 md:grid-cols-[1fr_0.75fr]">
        <div>
          <h1 className="max-w-[760px] text-[58px] font-bold leading-[1.08] tracking-[-0.025em] text-white sm:text-[70px] md:text-[76px]">
            {TICKETING_PAGE.title}
          </h1>
          <p className="mt-10 text-lg font-bold text-white">{TICKETING_PAGE.overviewLabel}</p>
          <div className="mt-4 h-px max-w-[650px] bg-white/45" />
          <p className="mt-6 max-w-[620px] text-lg font-bold leading-snug text-white">{TICKETING_PAGE.overview}</p>
        </div>
  ...
  return (
    <div className="bg-white text-black">
      ...
      <main className="mx-auto grid max-w-[1120px] gap-10 px-5 py-6 sm:px-8 lg:grid-cols-[1.05fr_1fr] lg:px-0">
        <TeamMembersSection members={TICKETING_PAGE.teamMembers} />
        <div>
      ...
      <UrgentUpcomingSection events={TICKETING_PAGE.events} />
    </div>
  );
}
